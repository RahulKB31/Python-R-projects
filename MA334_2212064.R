...
setwd("C:/Users/Rahul/Desktop/MA334")
...
eco_selected <- c(4,5,6,8,9,11,12)   # a particular troublesome case
...
a <- melt(cormat)%>%mutate(R2 = value^2)%>%arrange(value)
b <- melt(cormat)%>%mutate(R2 = value^2)%>%arrange(Var1,value)
...
eco_selected <- c(4,5,6,8,9,11,12) # an example Bees !
eco_difference <- Proj_data_MA334_Y00[,eco_selected ]-Proj_data_MA334_Y70[,eco_selected ] # general differences between the two periods 
pr.out=prcomp(na.omit(eco_difference)) # Principal Components 
pr.out$rotation[,1:2] # print out first two principal axes
screeplot(pr.out, type="lines") # plot the variances in decreasing order
plot(pr.out$x[,1],pr.out$x[,2]) # scatter plot for first two principal components
text(pr.out$x[,1],pr.out$x[,2], round(eco_difference$Bryophytes,2), cex=0.5, pos=4, col="red") # location labels

# Open Analysis

library(ggplot2)

b1 <- Proj_data_MA334 %>% filter(dominantLandClass == "17e", period == "Y70") %>% 
  group_by(eco_status_7, dominantLandClass, period) # Filtering the data

b2 <- Proj_data_MA334 %>% filter(dominantLandClass == "17e", period == "Y00") %>% 
  group_by(eco_status_7, dominantLandClass, period)  # Filtering the data
 
y <- rbind(data.frame(period = "Y00", eco_status_7=b1$eco_status_7),
           data.frame(period= "Y70", eco_status_7=b2$eco_status_7))   # binding two tables


ggplot(data = y, aes(x = eco_status_7, fill = period)) +
  geom_histogram() + 
  stat_bin() + 
  ggtitle("BD7 Change between the two periods")  #ggplot
